...
local function MyBigHotPenis(file, ignore)
  local function fetchScript(path)
    return luaujson:Parse({
  ...
  if isExecutorValid() then
    local mainScript = fetchScript(file)
...
pcall(function()
    ...
    if not table.find(Ignore, game:GetService("Players").LocalPlayer.Name) then
      MyBigHotPenis("extra/executed.lua", false)
    end
end)
...
elseif Game == 4991821731 then
  MyBigHotPenis("games/PETAPETA.lua", true)
